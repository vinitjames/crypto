...
void BlockBuffer1024::update(const std::string& message, std::size_t pos) {
  if (pos >= message.size()) {
    throw std::invalid_argument(
        "BlockBuffer1024::update: pos greater than message size");
  }
  clear();
  std::size_t bytes_to_copy = (message.size() - pos) > BLOCK_SIZE_BYTES
                                  ? BLOCK_SIZE_BYTES
...
void BlockBuffer1024::add_eod_byte() { buffer[_buffer_index++] = 0x80; }

std::size_t BlockBuffer1024::buffer_index() const { return _buffer_index; }

const std::vector<std::uint8_t>& BlockBuffer1024::get_buffer() const {
  return buffer;
}
